...
const PinehurstVillage: React.FC = () => {
  const community = COMMUNITIES_DATA['pinehurst-village'];
  ...
  useEffect(() => {
    document.title = "Pinehurst Village | New Homes in Felton, DE | Rush Home Team";
    window.scrollTo(0, 0);
  }, []);
  ...
  const nextLightbox = () => setLightboxIndex((prev) => (prev + 1) % community.gallery.length);
  const prevLightbox = () => setLightboxIndex((prev) => (prev - 1 + community.gallery.length) % community.gallery.length);
    ...
    <div className="bg-white min-h-screen font-['Montserrat']">
      ...
      <section className="pt-[100px] bg-white pb-4">
        <div className="max-w-[1200px] mx-auto px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-2 rounded-xl overflow-hidden h-[400px]">
            <div className={`relative cursor-pointer overflow-hidden group flex items-center justify-center ${community.mainImageContain ? 'bg-gray-50' : ''}`} onClick={() => openLightbox(0)}>
              <img 
                src={community.gallery[0]} 
                alt="Main Community View" 
                referrerPolicy="no-referrer"
                className={`w-full h-full transition-transform duration-300 group-hover:scale-[1.02] ${community.mainImageContain ? 'object-contain p-12' : 'object-cover'}`} 
              />
            </div>
              ...
              {[1, 2, 3, 4].map((i) => (
                community.gallery[i] && (
                  <div key={i} className="relative overflow-hidden cursor-pointer group" onClick={() => openLightbox(i)}>
                    <img src={community.gallery[i]} alt={`Gallery View ${i}`} referrerPolicy="no-referrer" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                  </div>
                )
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 text-[0.85rem] text-gray-500 mt-6 pb-4 border-b border-gray-200">
            ...
            <span>→</span>
            <strong className="text-black font-bold">Pinehurst Village</strong>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start py-8 gap-8">
            <div className="flex-1">
              ...
              <h1 className="text-[2.25rem] font-extrabold text-black leading-tight mb-2">Pinehurst Village</h1>
              ...
              <div className="flex flex-wrap gap-8 mt-6">
                ...
                <div className="w-[1px] bg-gray-200 hidden sm:block"></div>
                <div className="flex flex-col"><span className="text-[1.25rem] font-bold text-black">{community.stories}</span><span className="text-[0.8rem] text-gray-500 uppercase font-medium">Stories</span></div>
                <div className="w-[1px] bg-gray-200 hidden sm:block"></div>
      ...
      <nav className="sub-nav">
        <div className="max-w-[1200px] mx-auto flex px-8 overflow-x-auto whitespace-nowrap">
          {['Overview', 'Site Map', 'Location'].map((item) => (
            <a key={item} href={`#${item.toLowerCase().replace(/\s+/g, '')}`} onClick={(e) => scrollToSection(e, item.toLowerCase().replace(/\s+/g, ''))} className={`py-4 px-6 text-[0.9rem] font-bold transition-all border-b-[3px] border-transparent text-gray-500 hover:text-black`}>
              {item}
            </a>
          ))}
        </div>
        ...
        <div className="max-w-[1200px] mx-auto px-8 grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-16">
          <div>
            ...
            <div className="flex items-center gap-4 p-5 bg-gray-50 rounded-xl mt-8 border border-gray-100">
              <div className="w-14 h-14 bg-white border border-gray-200 rounded-lg flex items-center justify-center font-black text-[0.7rem] text-gray-500 uppercase">
              ...
              <div>
                <h4 className="font-bold text-[1rem]">Built by {community.builderName}</h4>
                <p className="text-[0.85rem] text-gray-500 font-medium">{community.builderExperience}</p>
              </div>
            </div>
          </div>
        ...
        <section className="py-16 bg-gray-50" id="sitemap">
          <div className="max-w-[1200px] mx-auto px-8">
            ...
            <div className="rounded-xl overflow-hidden shadow-lg h-[500px] bg-white">
              <iframe src={community.siteMapUrl} className="w-full h-full border-none" title={`${community.name} Site Map`}></iframe>
            </div>
          </div>
        </section>
      )}
        ...
        <div className="max-w-[1200px] mx-auto px-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-gray-50 p-8 rounded-xl border border-gray-100 shadow-sm">
             ...
             <div className="grid gap-2">
                {community.nearbyPlaces.map((p, i) => (
                  <div key={i} className="bg-white p-4 rounded-lg border border-gray-100 font-medium text-[0.9rem] text-gray-700 flex items-center gap-3">
                    <span className="text-[#d4a84b]">
                      {p.icon === 'beach' ? <Palmtree size={18}/> : p.icon === 'mall' ? <Building2 size={18}/> : p.icon === 'health' ? <HeartPulse size={18}/> : <MapPin size={18}/>}
                    </span>
      ...
      <section className="py-24 bg-[#111111] text-center text-white">
        <div className="max-w-[750px] mx-auto px-8">
          <h2 className="text-[2.75rem] font-black mb-4">Start Your New Chapter</h2>
          <p className="text-gray-400 mb-10 text-[1.15rem]">Experience the community atmosphere and active lifestyle at Pinehurst Village. Schedule your tour today.</p>
      ...
      {isLightboxOpen && (
        <div className="lightbox-custom" onClick={() => setIsLightboxOpen(false)}>
          ...
          <img src={community.gallery[lightboxIndex]} referrerPolicy="no-referrer" className="max-w-[90%] max-h-[85vh] object-contain rounded-lg" onClick={e => e.stopPropagation()}/>
          ...
          <div className="absolute bottom-10 text-white font-bold">{lightboxIndex + 1} / {community.gallery.length}</div>
        </div>
      )}
    </div>
  );
...
export default PinehurstVillage;