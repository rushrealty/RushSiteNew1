...
const CommunityCard: React.FC<CommunityCardProps> = ({ community, onClick }) => {
    ...
    <>
      ...
      <div className="p-4 flex flex-col flex-grow">
        <div className="mb-4">
             ...
             <h3 className="font-serif text-lg font-bold text-gray-900 leading-tight group-hover:text-compass-gold transition-colors mb-2">
               {community.name}
             </h3>
  ...
  // If onClick is provided, use it (for modal)
  if (onClick) {
    return (
      <div
  ...
  // Otherwise, if has slug, wrap in Link for navigation
  if (community.slug) {
    return (
      <Link
  ...
  // Fallback div (no click behavior)
  return (
    <div className="group bg-white rounded-3xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:shadow-[0_20px_40px_-5px_rgba(0,0,0,0.1)] transition-all duration-300 overflow-hidden flex flex-col h-full relative border border-gray-100/50">
      {CardContent}
    </div>
  );
};
