...
const ListingsPageContent: React.FC<ListingsPageContentProps> = ({ config, onPropertyClick, initialPropertyId, initialSearchTerm, initialCounty }) => {
  ...
  // Sync search term to autocomplete hook
  useEffect(() => {
    autocomplete.setQuery(searchTerm);
  }, [searchTerm]);
    ...
    <div className="pt-24 min-h-screen flex flex-col bg-white">
       ...
       <div className="bg-white py-8 px-4 border-b border-gray-100 shrink-0">
          <div className="max-w-4xl mx-auto text-center">
              <span className="text-compass-gold font-bold tracking-[0.25em] uppercase text-xs mb-3 block animate-fade-in">{config.heroLabel}</span>
              <h1 className="text-3xl md:text-4xl font-serif font-bold text-gray-900 mb-4 leading-tight animate-slide-up">{config.heroTitle}</h1>
       ...
       <div className="sticky top-20 md:top-24 z-40 bg-white border-b border-gray-200 shadow-sm shrink-0 overflow-visible">
          <div className="mx-auto px-4 sm:px-6 lg:px-10 xl:px-16 py-4 overflow-visible">

             {/* Filter Bar: Search | Toggles on mobile, full row on desktop */}
             <div className="flex items-center gap-3 lg:gap-6">

                {/* LEFT: Search Input with Autocomplete */}
                <div className="relative flex-1 min-w-0 lg:flex-none lg:w-[340px] xl:w-[420px] 2xl:w-[480px]">
             ...
             {/* Mobile filter panel - full-width using native selects for reliability */}
             {showFiltersMobile && (
               <div className="lg:hidden mt-4 pt-4 border-t border-gray-100 space-y-4">

                 {/* Row 1: Price Range */}
                 <div>
                   <label className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-1.5 block">Price Range</label>
                   <div className="flex items-center gap-2">
                     <select
                       className="flex-1 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-1 focus:ring-black outline-none"
                       value={priceMin ?? ''}
                       onChange={e => {
                         const val = e.target.value ? Number(e.target.value) : null;
                         setPriceMin(val);
                         if (val !== null && priceMax !== null && priceMax < val) setPriceMax(null);
                       }}
                     >
                       {MIN_PRICES.map(p => (<option key={p.label} value={p.value ?? ''}>{p.label}</option>))}
                     </select>
                     <span className="text-xs text-gray-400 font-semibold">to</span>
                     <select
                       className="flex-1 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-1 focus:ring-black outline-none"
                     ...
                     >
                       {MAX_PRICES.map(p => (
                         <option key={p.label} value={p.value ?? ''} disabled={priceMin !== null && p.value !== null && p.value < priceMin}>{p.label}</option>
                       ))}
                     </select>
                   </div>
                 </div>

                 {/* Row 2: Beds, Baths, County */}
                 <div className="grid grid-cols-3 gap-3">
                   <div>
                     <label className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-1.5 block">Beds</label>
                     <select
                       className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-1 focus:ring-black outline-none"
                       value={minBeds}
                   ...
                   <div>
                     <label className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-1.5 block">Baths</label>
                     <select
                       className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-1 focus:ring-black outline-none"
                       value={minBaths}
                   ...
                   <div>
                     <label className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-1.5 block">County</label>
                     <select
                       className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-1 focus:ring-black outline-none"
                       value={selectedCounties.length === 1 ? selectedCounties[0] : ''}
                       onChange={e => setSelectedCounties(e.target.value ? [e.target.value] : [])}
                     >
                 ...
                 {/* Row 3: Home Type checkboxes */}
                 <div>
                   <label className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-1.5 block">Home Type</label>
                   <div className="flex gap-2">
                     {HOME_TYPES.map(type => {
                       const isActive = selectedHomeTypes.includes(type);
                       return (
                 ...
                 {/* Row 4: Sqft Range */}
                 <div>
                   <label className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-1.5 block">Square Feet</label>
                   <div className="flex items-center gap-2">
                     <select className="flex-1 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-1 focus:ring-black outline-none" value={sqftMin ?? ''} onChange={e => setSqftMin(e.target.value ? Number(e.target.value) : null)}>
                       {SQFT_MIN_OPTIONS.map(o => (<option key={o.label} value={o.value ?? ''}>{o.label}</option>))}
                     </select>
                     <span className="text-xs text-gray-400 font-semibold">to</span>
                     <select className="flex-1 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:ring-1 focus:ring-black outline-none" value={sqftMax ?? ''} onChange={e => setSqftMax(e.target.value ? Number(e.target.value) : null)}>
                 ...
                 {hasActiveFilters && (
                   <button
                     ...
                     className="w-full py-2.5 text-sm font-semibold text-gray-500 hover:text-black flex items-center justify-center gap-1.5 border border-gray-200 rounded-lg"
                   >
       ...
       <div className="flex-grow flex relative">
          ...
          <div className={`w-full lg:w-[40%] bg-gray-200 relative ${viewMode === 'map' ? 'block' : 'hidden lg:block'}`}>
             <div className="h-[calc(100vh-200px)] lg:sticky lg:top-[136px] lg:h-[calc(100vh-136px)]">
